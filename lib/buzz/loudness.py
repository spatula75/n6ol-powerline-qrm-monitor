...
# of reporting silence rather than a threshold of our own choosing.
#
...
def measure(path: Path | str, ffmpeg: str) -> Loudness:
    ...
    the threshold, so a small difference in the ungated mean moves the gate, excludes
    more quiet blocks, raises the mean, and moves the gate again.  Small
...
def auto_gain_db(loudness: Loudness, target_lufs: float = TARGET_LUFS,
    ...
    and +16.4 dB -- but on one of them the ceiling permitted only +19.5, so it came
...
def resolve_gain(path: Path | str, ffmpeg: str) -> float:
    ...
    Logged at info because it is the one thing an operator will want to check when a
    render comes out louder or quieter than expected, and because the probe takes a
    couple of seconds during which nothing else appears to be happening.
    """
