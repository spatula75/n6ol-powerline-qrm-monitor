...
S1_DBM = S9_DBM - 8 * DB_PER_S_UNIT      # -121 dBm (S1)
S9P60_DBM = S9_DBM + 60                  # -13 dBm  (S9+60)
...
SCALE_ROW = '[' + ''.join(_SCALE_MARKS.get(i, ' ') for i in range(_BAR_WIDTH)) + ']'
TENS_ROW = ' ' + ''.join(_TENS_MARKS.get(i, ' ') for i in range(_BAR_WIDTH)) + ' '


def s_meter_bar(dbm: float) -> str:
    """Render `dbm` as a filled/empty ASCII bar, `_BAR_WIDTH` characters wide.

    Piecewise fill matching the two-section scale layout above:
    ...
    """
    if dbm <= S9_DBM:
        filled = min(_S1_TO_S9_CHARS, max(0, int((dbm - S1_DBM) // DB_PER_S_UNIT) + 1))
    else:
        fraction = min(1.0, (dbm - S9_DBM) / (S9P60_DBM - S9_DBM))
        filled = _S1_TO_S9_CHARS + int(fraction * _S9_TO_S9P60_CHARS)
    filled = max(0, min(filled, _BAR_WIDTH))
    return '█' * filled + '░' * (_BAR_WIDTH - filled)


def dbm_to_s_string(dbm: float) -> str:
    """The S-meter reading `dbm` corresponds to, the way a ham reads it off a dial.

    S1 through S9 in DB_PER_S_UNIT steps, then the wider spacing IARU uses above S9:
    S9+10, S9+20, and so on in 10 dB steps.
    """
    if dbm >= S9_DBM:
        over = dbm - S9_DBM
        if over < 10:
