...
Everything here is pure signal processing with no I/O or threading.  It builds
the pps scan kernel, scores how well a pulse train fits at each phase offset
(FFT convolution), averages amplitude at the pulse positions (Numba JIT), and
converts amplitudes to dBFS.  analyze_window() ties them together.  Given one
window of rectified audio, it finds the best and worst pulse phases and
measures the amplitude at each.  This is the single analysis step both the
continuous analyzer and the golden-file tests build on.
"""
...
# so it is unambiguously a sentinel and is never confused with a real reading.
SILENCE_DBFS = -128.0
...
def pulse_phase_period(sample_rate: int, pulse_rate: int) -> int:
    ...
    This, not samples_per_pulse, is the correct modulus for a phase.  Two start
    offsets one full period apart sample identical positions.  Offsets one
    samples_per_pulse apart differ by a sample on two thirds of the pulses.  Being
    an integer also keeps the phase arithmetic exact.  Reducing a float argmax by
    a fractional modulus can fall a hair under the true value (4.999999999997698)
    and then truncate to the wrong phase.
    """
...
# The signature is given explicitly, which makes numba compile at import rather than
# on the first call.  Lazy compilation runs on whichever thread gets there first.  In
# the GUI that thread is Qt mid-paint, which froze the window for about a second
# while audio kept arriving, and put a frozen second at the head of every rendered
# video.  At import it costs the same second where nothing is watching.
#
# There is one signature, because production uses only the one.  This was verified,
# not assumed: instrumented over a replayed .wav and over the live sound card, the
# window arrives as float32 every time -- np.abs(int16 samples - a float32 DC
# estimate).  Declaring it also means a caller passing anything else fails loudly
# here, rather than quietly compiling a second variant mid-flight at whatever moment
# it first happens.
#
...
def average_pulse_amplitude(mono_amplitude_array: np.ndarray, samples_per_pulse: float,
    ...
    the caller passes it in rather than deriving it from sample_rate / pulse_rate,
    because the true spacing depends on the actual grid frequency.  If the grid is
    off by even 0.05 pps, sampling at the nominal spacing walks 6.7 samples away
    from the real pulses over a one-second window.  That costs about 6 dB.  A caller
    that has measured the drift should pass the corrected spacing - see
    ContinuousAnalyzer._effective_samples_per_pulse().

    n_pulses is the number of pulse positions to average.  This samples
    PULSE_WIDTH_SAMPLES adjacent values per pulse position, then divides by the
    total sample count.  That is equivalent to correlating with the pulse kernel but
    without multiplying through the ~97% zeros: build_pulse_kernel() places its
    ones at the same round()ed positions, so the two agree sample for sample.

    This internally clamps n_pulses so the last access (start_index + pos + width -
    1) cannot exceed the array bounds regardless of what the caller passes.  It
    returns 0.0 when there is insufficient room for even one pulse group.

    The mean comes back as a float, not floor-divided to an integer.  Near the noise
    floor a mean-absolute amplitude of 1 LSB is only ~-90 dBFS, where one integer
    step is 6 dB.  Quantizing here would coarsen exactly the weak-signal readings the
...
def build_pulse_kernel(sample_rate: int, pulse_rate: int,
    ...
    matters: samples_per_pulse is 133.333 at 16 kHz / 120 pps, and truncating
    instead would put a third of the positions one sample away from where the
    amplitude averager reads them.  At PULSE_WIDTH_SAMPLES = 3 a one-sample offset
    costs a third of that pulse's energy, so the FFT would be optimizing a phase
    that the subsequent measurement never actually samples.
...
def calculate_pps_fit_array(mono_amplitude_array: np.ndarray, kernel: np.ndarray,
    ...
    fftconvolve computes convolution, which reads the kernel backwards.  Reversing
    the kernel first yields true cross-correlation: fit[n] is exactly the sum
    average_pulse_amplitude() would compute at start_index = n.  (The kernel used to
    be built as a palindrome so the reversal could be skipped, but that only held for
    rate pairs where the truncated pulse positions happened to be symmetric.)

    mode='valid' returns only positions where the kernel fits entirely within the
    signal, avoiding edge artifacts from the convolution.
    """
...
def analyze_window(mono_amplitude_array: np.ndarray, sample_rate: int, pulse_rate: int,
    ...
    This scores every phase offset with the FFT fit, takes the best-fit phase as
    the signal and the worst-fit phase as the noise reference, then averages the
    amplitude at each across the window.  Using the minimum-correlation phase as
    the noise reference is deliberate.  Sampling at the positions LEAST correlated
    with the pulse train measures everything except the powerline interference -
    atmospheric noise, man-made QRM, receiver thermal noise - so the caller can
    judge whether powerline noise is actually the dominant problem.
