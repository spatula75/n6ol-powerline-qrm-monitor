...
a single loud noise spike cannot fool it, and it holds through fades because the
drift tracker drives it rather than any individual pulse.
...
phases by.  That is what makes the mapping from phase to screen position exact
regardless of how that modulus relates to the sweep's own width.  The two coincide
at the defaults but not in general; sweep_geometry's docstring covers the case
where they differ.

The sweep is shorter than the frame interval, so each frame overlays several real
sweeps (4 at 25 ms per sweep and _UPDATE_MS = 100).  That gives the authentic
multi-trace look of an analogue scope, and it is also the reason no audio goes
unrendered: the same argument _mean_spectrum_db makes for averaging every FFT
frame per waterfall row rather than rendering only the newest one.

Persistence carries real information here.  The bright core of the trace is where
successive sweeps agree, and the dim halo around it is where they disagree, so the
halo's vertical width reads out pulse-to-pulse amplitude scatter directly and its
horizontal width reads out timing jitter.
...
rasterization and color math below it are plain functions with no Qt dependency,
and test_scope_math.py unit tests them.
"""
...
# persists after it, because the arc reaches us through the receiver's audio
...
# Intensity deposited by one sweep.  Below 1.0, so a single sweep renders as
# mid-scale on the color ramp and only agreement between several sweeps drives
# the trace to its hot white-green core.  That is what makes the core/halo
# distinction carry information instead of everything saturating on the first
# sweep.
...
# The pulses are the point of the display, and they occupy only ~2% of samples
# (3 samples out of every 133).  The statistic therefore has to reach well into
# the upper tail to see them at all - a p90 would describe the noise between
# pulses and scale the pulses clean off the screen.  p99.5 sits above the pulse
# population while still being a percentile rather than a bare max, so one
# sample of static crash cannot rescale the whole display on its own.
...
def sweep_start_offset(trigger_phase: int, pretrigger: int, sweep_samples: int) -> int:
    ...
    in general.  They differ in both directions: the phase period is shorter at
    ...
    the sweep width.  sweep_geometry allows for it with a whole phase period of
    slack at the head of the capture.
    """
...
class SweepGeometry:
    ...
    and blurs the trace, and neither of those announces itself.
    """
...
def sweep_geometry(sample_rate: int, pulse_rate: int) -> SweepGeometry:
    ...
    Everything is counted in pulse periods and then converted, which is what keeps
    the display saying the same thing everywhere: _SWEEP_PULSES cycles across the
    screen and _PHOSPHOR_PULSES cycles of persistence, at any rate and either grid
    frequency.

    The one thing that cannot be chosen freely is the *stride*.  Successive sweeps
    have to begin a whole phase period apart, because that is the only interval
    after which the pulse grid repeats against the sample clock.  Step by anything
    else and each sweep samples the pulses at a slightly different sub-sample
    offset, so they no longer superimpose and the trace smears.  The phase period is
    often shorter than the sweep (one pulse period at 48 kHz / 120 pps), so the
    stride is the fewest whole phase periods that reach the sweep width.
    Consecutive sweeps can therefore overlap in the audio, exactly as a real scope
    re-triggers on a running signal.
    """
...
def full_scale_dbfs(full_scale: float) -> float:
    ...
    This reports the deflection at the top of the graticule rather than a
...
def trace_rows(values: np.ndarray, full_scale: float, height: int,
    ...
    and centering it would waste the lower half of the screen on territory it can
    never reach.
...
def accumulate_trace(phosphor: np.ndarray, rows: np.ndarray, intensity: float) -> None:
    ...
    the signal moves faster than one row per column, which for a noisy pulse is most
    of the screen.
...
def build_graticule(height: int, width: int) -> np.ndarray:
    ...
    Composited under the trace with a maximum rather than drawn over it, so the
    grid sits on the CRT face and any real signal outranks it.  A graticule
...
def update_running_average(average: np.ndarray | None, sweeps: np.ndarray,
    ...
    Averaging runs on the *rectified* sweep, never on the raw bipolar waveform.  The
    arc is not phase-locked to the receiver's BFO, so the audio-frequency carrier
...
class ScopeWidget(QWidget):  # pragma: no cover -- requires a live Qt display
    ...
    def _tick(self) -> None:
        ...
        # The sound card's DC offset would sit the whole trace off the center line
        # and, in averaging mode, add a constant pedestal to the rectified envelope.
        # This uses the median rather than the mean, because the pulses themselves
        # would drag a mean (see ContinuousAnalyzer._capture).  No EMA smoothing is
        # needed at this window size: 2400 samples is a stable estimate on its own,
        # and unlike the analyzer this value is never used for a measurement anyone
        # reports.
    ...
    def _draw(self, buffer: np.ndarray, values: np.ndarray, full_scale: float,
        ...
        """Rasterize one trace into `buffer`.

        This asks for one extra column of row positions, so N+1 positions give N
        segments - see accumulate_trace.
        """
