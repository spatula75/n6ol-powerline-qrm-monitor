...
class TestTheNotebookDeclaresItsProvenance:
    """docs-notebook/ holds measurements and decisions, much of it model-written, and
    a reader deciding how far to trust a figure needs to know what produced it.

    Mechanical because it is easy to forget and forgetting is invisible: the rule was
    broken within minutes of being written, in the README that states it.
    """

    def _file(self, tmp_path, text, name='note.md'):
        """A file lint_file will see as living in the notebook folder."""
        folder = tmp_path / 'docs-notebook'
        folder.mkdir(exist_ok=True)
        (folder / name).write_text(text, encoding='utf-8')
        return Path('docs-notebook') / name

    def test_a_notebook_file_without_it_is_reported(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        path = self._file(tmp_path, '# A note\n\nSomething measured.\n')
        assert 'missing attribution' in rules(lint_file(path))

    def test_a_notebook_file_with_it_passes(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        path = self._file(tmp_path, 'Generated by Claude.\n\n# A note\n\nMeasured.\n')
        assert 'missing attribution' not in rules(lint_file(path))

    def test_leading_blank_lines_do_not_hide_it(self, tmp_path, monkeypatch):
        """The first line carrying anything is what counts, not the first byte."""
        monkeypatch.chdir(tmp_path)
        path = self._file(tmp_path, '\n\nGenerated by Claude.\n\n# A note\n')
        assert 'missing attribution' not in rules(lint_file(path))

    def test_an_empty_notebook_file_is_reported(self, tmp_path, monkeypatch):
        """Rather than crashing on an empty splitlines()."""
        monkeypatch.chdir(tmp_path)
        path = self._file(tmp_path, '')
        assert 'missing attribution' in rules(lint_file(path))

    def test_files_elsewhere_are_left_alone(self, tmp_path, monkeypatch):
        """The rule is about this folder.  README.md at the root is not a notebook."""
        monkeypatch.chdir(tmp_path)
        (tmp_path / 'README.md').write_text('# Project\n\nA sentence.\n', encoding='utf-8')
        assert 'missing attribution' not in rules(lint_file(Path('README.md')))

    def test_every_notebook_file_in_this_repo_carries_it(self):
        """The check guards the rule; this guards the repository."""
        folder = TOOL.parent.parent / 'docs-notebook'
        files = sorted(folder.glob('*.md'))
        assert files, 'docs-notebook/ has no documents, so this test guards nothing'
        for path in files:
            first = path.read_text(encoding='utf-8').lstrip().splitlines()[0]
            assert first.startswith('Generated by'), (
                f'{path.name} opens with {first!r} rather than an attribution line')


class TestSubjectlessOpenersAreReported:
    """A sentence opening with a participle and never saying who acted.

    "Snapped to the nearest step the tuner offers" shipped as the description of
    [rtlsdr] gain_db, where an operator reads it.  It is not the fragment case: it has
    two finite verbs, both in subordinate clauses, which is exactly why the fragment
    check reported it clean.  The defect is a main clause with no subject.
    """

    def _schema(self, tmp_path, description, field='gain_db'):
        body = {'properties': {'rtlsdr': {'properties': {
            field: {'type': 'number', 'title': 'Tuner gain (dB)',
                    'description': description, 'default': 40.2}}}}}
        path = tmp_path / 'schema.json'
        path.write_text(json.dumps(body, indent=2), encoding='utf-8')
        return path

    def test_the_description_that_prompted_this_is_reported(self, tmp_path):
        path = self._schema(
            tmp_path,
            'Snapped to the nearest step the tuner offers, since it accepts only a '
            'fixed set.')
        assert any('names no subject' in f.rule for f in fragments_in(path))

    def test_the_fragment_check_alone_would_miss_it(self, tmp_path):
        """The reason this check exists as well as that one.  If this ever starts
        passing, the two have merged and one of them is redundant.
        """
        sentence = ('Snapped to the nearest step the tuner offers, since it accepts '
                    'only a fixed set.')
        assert looks_like_a_fragment(sentence) is False
        assert looks_subjectless(sentence) is True

    def test_naming_the_actor_clears_it(self, tmp_path):
        path = self._schema(
            tmp_path,
            'The monitor snaps this to the nearest step the tuner offers, since the '
            'tuner accepts only a fixed set.')
        assert not any('names no subject' in f.rule for f in fragments_in(path))

    def test_an_imperative_is_not_reported(self, tmp_path):
        """"Set 0 to record every event" is correct prose and opens with a word that
        is a participle in other company.  The number after it is what separates them,
        which is why the tokenizer has to see digits.
        """
        path = self._schema(tmp_path, 'Set 0 to record every event until you stop.')
        assert not any('names no subject' in f.rule for f in fragments_in(path))

    def test_a_field_title_is_not_reported(self, tmp_path):
        """Titles are noun phrases by house convention, so flagging one would report
        the style as an error.  "Calibrated at gain (dB)" is a real title here.
        """
        body = {'properties': {'rtlsdr': {'properties': {'calibrated_at_gain_db': {
            'type': 'number', 'title': 'Calibrated at gain (dB)',
            'description': 'The tuner gain the calibration used.', 'default': 1.0}}}}}
        path = tmp_path / 'schema.json'
        path.write_text(json.dumps(body, indent=2), encoding='utf-8')
        assert not any('names no subject' in f.rule for f in fragments_in(path))

    def test_the_notebook_attribution_line_is_exempt(self):
        """_check_attribution requires it in exactly this form, so reporting it would
        make the tool argue with itself.
        """
        assert looks_subjectless('Generated by Claude.') is False

    def test_markdown_is_not_read_at_all(self, tmp_path):
        """A gap rather than a decision, and pinned so that closing it is deliberate.
        markdown_prose yields a line at a time, so a wrapped paragraph would report
        every half-sentence as a fragment.
        """
