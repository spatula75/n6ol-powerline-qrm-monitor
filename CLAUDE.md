...
- `docs-notebook/` - engineering notebook: why a constant holds the value it does,
...
8. **Documentation drift.** A code or behavior change means checking `README.md`, the
   ...
   **`docs-notebook/` is exempt from that approval step, and from nothing else.**
   It is the engineering notebook: why a constant holds the value it does, what an
   experiment ruled out, how the hardware behaved when tested. Write it down while it
   is fresh rather than waiting to be asked. The prose rules and `ste_lint` apply in
   full, because these documents are read at the moment somebody is trying to
   understand something difficult, and being internal is not a reason to write badly.

   Three rules for it. One topic per file, named for the topic rather than for a date
   or a number, since you will come looking for a subject. Put the date inside the
   document beside the measurements, because a figure without one cannot be judged.
   And **every file a model generates says so on its first line** - "Generated by
   Claude" is enough.

   That last one is not a contradiction of the no-self-attribution rule under Git
   workflow, which bans a byline on work that ships. This is provenance on a record
   of measurements, and a reader deciding how far to trust a figure needs to know
   what produced it. A model should also not be able to read its own document back
   later as though a person had written it.

   **Run `ste_lint` over every notebook document you write or change, at the time you
   write it, rather than leaving it to the pre-commit sweep.** These are long
   documents written in one pass, so a finding caught at the end means rereading the
   whole thing instead of the paragraph that was wrong.

   `ste_lint` enforces the attribution line itself, so a notebook file that does not
   open with "Generated by ..." is a finding like any other. That is deliberate: the
   rule was broken within minutes of being written, in the README that states it, by
   thinking of that file as structure rather than as content. A rule worth having and
   easy to forget belongs in the tool rather than in somebody's memory.

   Note that passing a *directory* to `ste_lint` checks nothing and reports clean, so
   name the files or use `--changed`.
...
**At the same moment, ask whether the work earned a `docs-notebook/` document.** That
is a separate question from the one above, and it has a different subject. This file
takes the rule that generalizes, where the notebook takes the evidence that does not:
what was measured, on what hardware, on what date, and what the numbers ruled out.
Ask, and write it on a yes - the notebook is exempt from the approval step, not from
being asked for.

Two things trigger it. Running an experiment is the first, whether it produced a
number the code now depends on or only ruled an approach out, because a failed
experiment repeated in six months costs exactly as much as it did the first time. A
design decision is the second, and most of all a decision *not* to build something,
since that one leaves behind no code where a comment could sit. "No device picker" and
"the offset dialog warns rather than refuses" had nowhere else to go.

The test is whether somebody, this project's author or a later session, could arrive
at the same question and have to redo the work to answer it. A number in a comment
says what the value is; the notebook says what was measured to get it, so that the
next person can judge the figure instead of trusting it. Where a measurement is quoted
in both places, note the document beside the constant, and remember that a fact stated
twice is a drift pin waiting to be written.
...
- **A config key that moves between sections silently loses whatever an operator set.**
  `_load_section` keeps only the keys the dataclass declares, so a key in the wrong
  section is dropped without a word and the default takes its place. Moving
  `pulse_rate` out of `[audio]` would leave a 50 Hz station's `pulse_rate = 100`
  ignored, and the monitor would hunt a 120 pps train that is not there, on a config
  file that still says 100. Nothing in the program can tell that apart from a file
  that never set it.

  So treat a key's section as part of its name. Where one has to move, read the old
  spelling as well as the new one for a release, warn when the old one appears, and
  say in the warning which section it belongs in now. Deleting a key is the same
  problem in a milder form and gets the same treatment, because an operator whose
  setting stopped working deserves to hear it from the program rather than from a
  measurement that looks wrong.
