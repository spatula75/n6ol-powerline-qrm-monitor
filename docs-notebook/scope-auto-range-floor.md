...
`buzz.display.scope.auto_range_full_scale` limits how far the scope magnifies a quiet input.
...
container. `buzz.display.scope.minimum_full_scale` then converts the combined resolution into
...
`tests/display/test_scope_math.py`.  The floor must clear what the receiver produces with no
