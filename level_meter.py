...
_S9_DBM = -73
_S1_DBM = _S9_DBM - 8 * 6      # -121 dBm (S1)
_S9P60_DBM = _S9_DBM + 60      # -13 dBm  (S9+60)
...
def _dbm_to_s_string(dbm: float) -> str:
    if dbm >= _S9_DBM:
        ...
        if over < 50:
            return 'S9+40'
        return 'S9+60'
    return f'S{max(1, round(9 + (dbm - _S9_DBM) / 6))}'


def _bar(dbm: float) -> str:
    # Piecewise fill matching the two-section scale layout:
    #   S1–S9  : 9 positions, one per S-unit (6 dB).  S_n lights exactly n positions.
    #   S9–+60 : 12 positions, one per 5 dB.  Each labeled mark (+20/+40/+60) lights
    #            exactly 4 more positions than the previous labeled mark.
    if dbm <= _S9_DBM:
        filled = min(_S1_TO_S9_CHARS, max(0, int((dbm - _S1_DBM) // 6) + 1))
    else:
        fraction = min(1.0, (dbm - _S9_DBM) / (_S9P60_DBM - _S9_DBM))
...
def main() -> None:
    ...
    print(f'Scale  : S1 = {_S1_DBM} dBm  -  S9 = {_S9_DBM} dBm  (each S-unit = 6 dB)')
    print()
